...
module.exports.run = async function({ api, args, Users, event, Threads, utils, client }) {
	...
	if (!info.adminIDs.some(item => item.id == api.getCurrentUserID())) return api.sendMessage('ðð‹ð„ð€ð’ð„ ðŒð€ðŠð„ ðŒð„ ð€ðƒðŒðˆð ð“ð‡ð„ð ð“ð‘ð˜', threadID, messageID);
	...
	*/
	...
	if(args[0] == "view") {
		if(!args[1]) {
			...
			if(!mywarn) return api.sendMessage('âœ…You have never been warned', threadID, messageID);module.exports.config = {
  name: "kick",
  version: "0.0.1",
  role: 2,
  credits: "Cliff",//do not change credits
  ...
  */
  ...
  if(args[0] == "view") {
    if(!args[1]) {
      ...
      api.sendMessage(`âŽYou have been warned for the reason : ${msg}`, threadID, messageID);
    }
    ...
    var info = await api.getThreadInfo(threadID);
  if (!info.adminIDs.some(item => item.id == senderID) && !(admin).includes(senderID)) return api.sendMessage('âŽRight cunt border!', threadID, messageID);

    if(!id) return api.sendMessage("âŽNeed to enter the id of the person to be removed from the banned list of the group", threadID, messageID);
    bans.banned;
    if(!mybox.includes(id)) return api.sendMessage("âœ…This person hasn't been banned from your group yet", threadID, messageID);
      api.sendMessage(`â”â€¢â”â€¢â”â€¢â” â—Ž â”â€¢â”â€¢â”â€¢â”“\nð’ð”ð‚ð‚ð„ð’ð’ð…ð”ð‹ð‹ ð‘ð„ðŒðŽð•ð„ðƒ ${id} \nâ”—â€¢â”â€¢â”â€¢â” â—Ž â”â€¢â”â€¢â”â€¢â”›  `, threadID, messageID);
  ...
  if (!info.adminIDs.some(item => item.id == senderID) && !(admin).includes(senderID)) return api.sendMessage('âŽRight cunt border!', threadID, messageID);
    ...
    api.sendMessage("Reset all data in your group", threadID, messageID);
     ...
     //â—†â”â”â”â”â”â”â”â”â”â—†WARNâ—†â”â”â”â”â”â”â”â”â”â—†\\
       ...
       var info = await api.getThreadInfo(threadID);
  if (!info.adminIDs.some(item => item.id == senderID) && !(admin).includes(senderID)) return api.sendMessage('Right cunt border!', threadID, messageID);
  var reason = "";
    ...
    fs.writeFileSync(__dirname + `/cache/bans.json`, JSON.stringify(bans, null, 2));
		...
		var id = parseInt(args[1]), mybox = bans.banned[threadID];
		var info = await api.getThreadInfo(threadID);
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('âŽRight cunt border!', threadID, messageID);
